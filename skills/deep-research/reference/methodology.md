...
### Resilient Fetching Protocol (WebFetch → Bright Data Fallback)

WebFetch frequently fails on sites with anti-bot protections, paywalls, or aggressive CDN rules. When a WebFetch call fails, retry with Bright Data's scraping tools as a fallback.

**Trigger conditions — retry with Bright Data when WebFetch returns ANY of:**
- HTTP 403 Forbidden
- HTTP 401 Unauthorized
- HTTP 429 Too Many Requests
- Empty or near-empty content (response exists but article body is missing — e.g., only nav/CSS/JS returned)
- Captcha or bot-detection page content (look for keywords: "captcha", "verify you are human", "access denied", "cloudflare", "just a moment")
- Explicit error messages: "Request failed with status code 4xx"

**Fallback chain:**
1. **First attempt:** `WebFetch(url, prompt)` — fast, no MCP overhead
2. **On failure:** `mcp__Bright_Data__scrape_as_markdown(url)` — headless browser scrape, returns clean markdown
3. **If markdown scrape also fails:** `mcp__Bright_Data__scrape_as_html(url)` then extract with a follow-up prompt

**Implementation rules:**
- Do NOT preemptively use Bright Data. Always try WebFetch first — it is faster and uses fewer resources.
- When falling back, use the SAME prompt/extraction goal. Do not change what you are looking for.
- If Bright Data returns content, process it identically to WebFetch results (same evidence extraction, same credibility scoring).
- Log which sources required fallback (include in methodology appendix: "N of M sources required Bright Data fallback due to anti-bot protections").
- If BOTH WebFetch and Bright Data fail for a source, skip it and note the gap. Do not retry more than once per tool per URL.

**For parallel deep-dive agents (Task tool sub-agents):**
Include this instruction in the agent prompt: "When fetching URLs, try WebFetch first. If it returns a 403, empty content, or captcha page, retry the same URL using mcp__Bright_Data__scrape_as_markdown. If that also fails, try mcp__Bright_Data__scrape_as_html and extract the relevant content. Do not retry more than once per tool."
