...
### Step 5: Multi-Model Code Review Debate (EXPERIMENTAL — Team of Specialists)

**Why:** Research shows multi-model review catches bugs single-model review misses.
Claude scores 0/2 on concurrency and compatibility bugs where Gemini catches them.
Claude+Gemini pair covers 91% of what a 5-model ensemble catches (Milvus benchmark, 2026).
Single-model self-review makes performance WORSE (arXiv Team of Rivals paper, 2026).

**When:** Run the debate on ALL code that handles: payment, auth, data mutations,
external API integration, and security-sensitive operations. For UI-only changes
or simple CRUD, the standard /tools:multi-agent-review is sufficient.

**How:** Run the review debate script on each critical file:

```bash
# Run multi-model debate on critical code
# Scripts auto-bootstrap their venv — no manual pip install needed.
# If OPENAI_API_KEY / GOOGLE_API_KEY are not set, scripts degrade to Claude-only review.
for file in $(find {workspace}/code/src -name "*.ts" -o -name "*.py" | head -20); do
  python3 {skill_dir}/scripts/review_debate.py \
    --input "$file" \
    --output "{workspace}/artifacts/tests/{epic}/debate_$(basename $file).json" \
    --agent-id "vp-eng-review" \
    --max-rounds 3 \
    --max-cost 3.00
done
```
...
**Reading the results:**
- `agreed_findings`: Both GPT and Gemini found the issue independently → HIGH confidence, fix immediately
- `disputed_findings`: One model found it, the other disagreed → include BOTH perspectives in the report, flag for human review
- `unique_findings`: Only one model found it → MEDIUM confidence, include in report
- `verdict: "needs_human_review"`: Unresolved disputes after 3 rounds → escalate with full debate history

**Fallback:** If OPENAI_API_KEY or GOOGLE_API_KEY are not set, fall back to standard single-model review:

### Step 5 (fallback): Run Multi-Agent Code Review
